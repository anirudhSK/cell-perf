...
int main() {
  ...
  Socket wifi_tx_sock;
  Socket::Address wifi_addr("192.168.5.1",18001);
 ...
 Socket::Address bcast("192.168.5.255",18000);
 uint64_t packets_sent=0;
 uint64_t last_time=0;
 while(1) {
  uint64_t the_time = Socket::timestamp();
  if ( the_time > last_time + 100e6 ) { /* 100 ms */
   packets_sent++;
   ...
   usleep((last_time + 100e6 - the_time)/(1.0e3)); /* ns to us conversion */
  }
 }
}
